  ...
  function Mask(mask, maskType)
  {
    this.mask = mask;
	this.type = (typeof maskType == "string") ? maskType : "string";
	this.error = [];
	this.errorCodes = [];
	this.value = "";
	this.strippedValue = "";
  ...
  Mask.prototype.attach = function (edit)
  {
	$addEvent(edit, "onkeydown", "return " + this.ref + ".isAllowKeyPress(event, this);", true);
	$addEvent(edit, "onkeyup", "return " + this.ref + ".getKeyPress(event, this);", true);
	$addEvent(edit, "onblur", "this.value = " + this.ref + ".format(this.value);", true);
  }
  Mask.prototype.isAllowKeyPress = function (event, edit)
  {
    if( this.type != "string" ) 
	  return true;
	var xe = new qEvent(event);
	if( ((xe.keyCode > 47) && (edit.value.length >= this.mask.length)) && !xe.ctrlKey ) 
	  return false;
	return true;
  }
  ...
  Mask.prototype.setGeneric = function (_v, _d)
  {
	...
	if( !this.allowPartial && !(new RegExp(rt.join(""))).test(v) ) 
	  ...
	  if( (this.allowPartial && (v.length > 0)) || !this.allowPartial )
	  {
  ...
  Mask.prototype.setNumber = function(_v, _d)
  {
	...
	if( (vd.length > 0 && !this.allowPartial) || ((dm.length > 0) && this.allowPartial && (v.indexOf(".") > -1) && (_vd.length >= vd.length)) )
	{
	  ...
	  if( (dm.length > 0) && this.allowPartial && (v.indexOf(".") > -1) && (_vd.length < vd.length) )
	  {
	    v = vi + "." + _vd;
	  } else 
	  {
  ...
  function $addEvent(obj, _event, sCmd, _bAppend)
  {
    var event = _event.toLowerCase();
    var bAppend = (typeof _bAppend == "boolean") ? _bAppend : true;
    var x = (obj[event]) ? obj[event].toString() : "";
    x = x.substring(x.indexOf("{")+1, x.lastIndexOf("}"));
    x = ((bAppend) ? (x + sCmd) : (sCmd + x)) + "\n";
    return obj[event] = (!!window.Event) ? new Function("event", x) : new Function(x);
  }
